...
<br>
<br>
> 친절한 docs에 의하면, Google Vision OCR API는 로컬 이미지 링크가 아닌 ```base64``` 형태로 이미지 링크를 넣어줘야 합니다. 멍청한 개발자는 이 사실을 모른 채, ```textDetection```에 로컬 링크를 함수 인자로 전달했습니다. 밑 빠진 독에 열심히 물을 퍼나른 셈이죠. 개발자는 재빨리 코드를 수정했습니다. <br>휴대폰 카메라로 사진을 찍고 이미지 링크를 API 함수로 넘겨줄 때, 로컬 링크가 아닌 base64로 인코딩하여 함수 인자를 전달했습니다. <br>로컬 이미지 링크를 base64로 인코딩하는 방법은 다양했습니다. react-native-base64 오픈 소스를 활용하면 쉽게 인코딩이 가능했죠. <br>하지만 이는 맥에서만 잘 돌아가고 윈도우에선 버전 호환 오류가 생겼습니다. ```React Native``` 버전과 ```react-native-base64```의 버전이 맞지 않아 설치가 되지 않았던 거죠. 다시 곤경에 빠진 개발자는 1초에 3개씩 팔린다는 서브웨이보다 더 많이 구글링이라는 발품을 팔았습니다. 다행히 해결책을 발견할 수 있었습니다. 등잔 밑이 어둡다는 옛 선조들의 말씀은 틀리지 않았습니다. <br>휴대폰 카메라로 사진을 찍을 때 사용되는 ```react-native-image-crop-picker```에서 imagePath를 생성할 때, ```includeBase64```를 **true**로 변경하면 이미지 링크가 base64 형태로 인코딩 되어 생성됩니다. 이로써 이미지 링크 오류를 해결할 수 있었습니다.
```sh
