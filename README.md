...
일치율을 계산하기 위해서 모든 식재료의 이름과 단위를 맞춰야 했습니다. <br><br>하지만 같은 식재료임에도 이름과 단위가 다양하여 통일하기가 번거로웠습니다. 그 이유는 사용자가 식재료를 추가할 때 직접 식재료명을 입력했고, 영수증을 통해 식재료를 인식했을 때 영수증마다 찍힌 식재료명과 단위가 달랐기 때문입니다. <br><br>그래서 저희는 일치율 계산에 사용되는 식재료명과 단위를 개발자가 직접 통일하여 데이터 베이스에 모아두었습니다.
